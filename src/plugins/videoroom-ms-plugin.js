...
/**
 * This module contains the implementation of the VideoRoom multistream plugin (ref. {@link https://janus.conf.meetecho.com/docs/videoroom.html}).
 * @module videoroom-ms-plugin
 */

import Handle from '../handle.js';

/* The plugin ID exported in the plugin descriptor */
const PLUGIN_ID = 'janus.plugin.videoroom-ms';
...
class VideoRoomHandle extends Handle {
    ...
    if (plugindata && plugindata.data && plugindata.data.videoroom) {
      ...
      switch (videoroom) {
        ...
        case 'joined':
          ...
          janode_event.data.publishers = message_data.publishers.map(({ id, display, streams }) => {
            const pub = {
              feed: id,
              display,
            };
   ...
   * @returns {Promise<module:videoroom-ms-plugin~VIDEOROOM_EVENT_PUB_JOINED>}
   */
  async joinPublisher({ room, feed, display, token, pin }) {
    const body = {
   ...
   */
  async joinConfigurePublisher({ room, feed, streams, bitrate, record, filename, display, token, pin, e2ee, jsep }) {
    const body = {
